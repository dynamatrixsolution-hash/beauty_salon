...
function normalizeFaqs(value: unknown): ServiceFaq[] {
  ...
  return value
    ...
    .filter((faq): faq is ServiceFaq => faq !== null);
}
    ...
    async function loadData() {
      try {
        ...
        if (svcData.success && svcData.data) {
          ...
          setPricing(svc.pricing !== undefined && svc.pricing !== null && svc.pricing !== 0 ? svc.pricing.toString() : '');
          setDescription(svc.description || '');
          setBenefits(svc.benefits || '');
          setImage(svc.image || '');
          setFeatured(!!svc.featured);
          setIsActive(svc.isActive !== undefined ? !!svc.isActive : true);
          setTags(svc.tags?.join(', ') || '');
          
          try {
            const parsedFaqs = JSON.parse(svc.faqs || '[]');
            setFaqs(normalizeFaqs(parsedFaqs));
          } catch {
            setFaqs([]);
          }
        } else {
          setError(svcData.error || 'Failed to load service details.');
        }
      } catch (err) {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        ...
        <p className="text-white/40 text-sm">Loading service data...</p>
      </div>
    );
  }
